...
namespace MatchingSystem.DataLayer.Feature.Repository
{
    public class MatchingRepository : ConnectionBase, IMatchingRepository
    {
        ...
        public IEnumerable<OldEntities.Matching> GetMatchings()
        {
            return GetMatchingsAsync().Result;
        }

        public async Task<IEnumerable<OldEntities.Matching>> GetMatchingsAsync()
        {
            return await Connection.QueryAsync<OldEntities.Matching>("select * from dbo.Matching");
        }
        ...
        public IEnumerable<OldEntities.Matching> GetMatchingsByUser(int userId)
        {
            return Connection.Query<OldEntities.Matching>("select MatchingID, MatchingName, MatchingTypeName_ru " +
                                                          "from napp.get_UserMatchings(@UserID, null)"
                , new {UserID = userId});
        }

        public async Task<IEnumerable<OldEntities.Matching>> GetMatchingsByUserAsync(int userId)
        {
            return await Connection.QueryAsync<OldEntities.Matching>("select MatchingID, MatchingName, MatchingTypeName_ru " +
                                                                     "from napp.get_UserMatchings(@UserID, null)"
                , new {UserID = userId});
        }
